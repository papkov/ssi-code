...
class UNet(nn.Module):
    def __init__(
        ...
        mode="nearest",
        residual=False,
        kernel_regularisation=False,
        num_internal_channels=8,
        ndim=2,
    ):
    ...
    def __init__(self, in_channels, out_channels, mode="bilinear", ndim=2):
        ...
        if mode == "bilinear" or mode == "nearest":
            ...
            self.conv = DoubleConv(
                in_channels, out_channels, in_channels // 2, ndim=ndim
            )
        else:
...
class OutConv(nn.Module):
    def __init__(self, in_channels, out_channels, ndim=2):
        ...
        if ndim == 2:
            self.conv = nn.Conv2d(
                in_channels, out_channels, kernel_size=1, padding=0, bias=True
            )
        elif ndim == 3:
            self.conv = nn.Conv3d(
                in_channels, out_channels, kernel_size=1, padding=0, bias=True
